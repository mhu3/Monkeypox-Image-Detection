...
def policy_v0():
  ...
  policy = [
      ...
      [('Brightness', 1.0, 7)],
      [('Sharpness', 1.0, 8)],
      [('TranslateX', 1.0, 1.5), ('Contrast', 1.0, 8)],
      [('Rotate', 1.0, 6), ('Brightness', 1.0, 4)],
      [('Rotate', 1.0, 5), ('Color', 1.0, 6)],
      [('FlipX', 1.0, 1)],
      [('FlipY', 1.0, 1)],
      [('Noise', 1.0, 5)],
      [('Blur', 1.0, 5)],
      [('Blur', 1.0, 9), ('FlipX', 1.0, 1)],
      ...
      # [('Posterize', 0.8, 2), ('Solarize', 0.6, 1)],
...
def noise(image, std):
  ...
  image = tf.convert_to_tensor(image)
  noise = tf.random.normal(tf.shape(image), mean=0.0, stddev=std,
                           dtype=tf.float32)
...
def blur(image, sigma):
  ...
  image = tf.convert_to_tensor(image)
  image = tf.cast(image, tf.float32)
  kernel = _gaussian_kernel(5, sigma, 3, tf.float32)
  strides = [1, 1, 1, 1]
  degenerate = tf.nn.depthwise_conv2d(
      tf.expand_dims(image, 0), kernel, strides, padding='SAME')
  degenerate = tf.clip_by_value(degenerate, 0.0, 255.0)
  return tf.cast(tf.squeeze(degenerate, [0]), tf.uint8)
...
def level_to_arg(params):
  return {
      ...
      'FlipX': lambda level: (),
      'FlipY': lambda level: (),
      'Noise': lambda level: (int((level/_MAX_LEVEL) * 20),),
      'Blur': lambda level: (int((level/_MAX_LEVEL) * 20),),
  }
